define [], () ->
  ...
  class InboxController
    ...
    constructor: (ngApp, taskManager, taskUtils) ->
      ngApp.controller "InboxController", ['$scope', '$rootScope', ($scope, $rootScope) ->
        inbox = taskManager.findById(taskManager.inboxId)
...
#console.log "tasks updated: ", tasks
        ...
        $scope.createInplaceTaskForm = ->
          ...
          inbox.children.push(taskManager.formTaskId)
          $scope.formTask.parent = inbox.id
          # emit taskchange
        ...
        $rootScope.$on 'taskChange', (event, flag) ->
          ...
          if !flag
            if inbox.children
              $scope.tasks = _(inbox.children).map (childId) ->
                taskUtils.buildTaskTree(taskManager.findById(childId))
